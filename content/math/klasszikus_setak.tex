...
The book Probability and Computing\cite{MitzenmacherProbability}, written by Michael Mitzenmacher and Eli Upfal, introduces random walks in the classical sense the following way.
...
Frequently studied characteristics of random walks are \textit{hitting time} and \textit{mixing time}. Informally, hitting time describes how quickly can a vertex be reached from another vertex in the graph, while mixing time expresses how fast the walk reaches the stationary distribution, where the starting vertex can no longer be identified.
...
\begin{definition}[Mixing time] The smallest time index of the Markov chain, where the total variational distance between the current and the stationary distribution is not greater than a given $\varepsilon$. This measure depends on the starting distrubiton $\pi_0$.

\begin{align}
m(\varepsilon, \pi_0) = \min\{t : \sum\limits_{j=0}^{n}|\pi_t[j] - \pi[j]| \leq \varepsilon\}
\end{align}
